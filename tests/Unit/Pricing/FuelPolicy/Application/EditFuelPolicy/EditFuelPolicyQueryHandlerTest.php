...
namespace App\Tests\Unit\Pricing\FuelPolicy\Application\EditFuelPolicy;
...
use Pricing\FuelPolicy\Application\EditFuelPolicy\EditFuelPolicyQuery;
use Pricing\FuelPolicy\Application\EditFuelPolicy\EditFuelPolicyQueryHandler;
use Pricing\FuelPolicy\Application\EditFuelPolicy\EditFuelPolicyResponse;
use Pricing\FuelPolicy\Domain\FuelPolicy;
use Pricing\FuelPolicy\Domain\FuelPolicyRepository;

class EditFuelPolicyQueryHandlerTest extends TestCase
{
    /**
    ...
    /**
     * @var EditFuelPolicyQueryHandler
     */
    private $editFuelPolicyQueryHandler;
    protected function setUp(){
        ...
        $this->editFuelPolicyQueryHandler = new EditFuelPolicyQueryHandler($this->fuelPolicyRepository, $this->complementRepository);
    }
    /**
     * @test
     */
    public function should_return_edit_fuelPolicy_response()
    {
        ...
        $response = $this->editFuelPolicyQueryHandler->handler(new EditFuelPolicyQuery(2));
        $this->assertInstanceOf(EditFuelPolicyResponse::class,$response);
        ...
        $this->assertEquals($response->getDataComplement(),
            [
            ["id" => 1, "commercialName" => "alias 1"],
            ["id" => 2, "commercialName" => "alias 2"],
            ["id" => 3, "commercialName" => "alias 3"],
            ["id" => 4, "commercialName" => "alias 4"],
            ["id" => 5, "commercialName" => "alias 5"],
            ["id" => 6, "commercialName" => "alias 6"],
            ["id" => 7, "commercialName" => "alias 7"],
            ["id" => 8, "commercialName" => "alias 8"],
        ]);
        $this->assertEquals([1,2,4,6], $response->getIdComplements());
    }
}