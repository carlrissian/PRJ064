...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(function () {
      this.filter = document.getElementById(this.filtersId +'filters');
      this.filterActive = document.getElementById('active-filters');
    })
  },
  methods: {
    clickButton() {
    ...
    showAndHide() {
      ...
      this.statusButtonRemove = this.items.length > 0 && this.filter.classList.contains('show');
      this.filter.classList.toggle('collapse');
      this.filterActive.classList.toggle('collapse');
    },
    fieldsIsEmpty() {
      let empty = true;
      this.elements.forEach( el => {
